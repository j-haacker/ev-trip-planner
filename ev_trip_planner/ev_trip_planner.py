...
class battery:
    def __init__(self, capacity, charging_rate: callable):
        self.capa = capacity
    ...
    def charge(
        self, stop_percentage: float = None, kWh: float = None, duration: float = None
    ) -> float:
        if stop_percentage:
        ...
        for i in range(duration * 60):
            self.state += (
                self.charging_rate(self.state) / 60 / 60 / self.capa * 100
            )  # per second
...
class vehicle:
    def __init__(
        ...
        if battery_ is None:
            self.battery = battery(
                battery_capacity_kWh, battery_charging_rate_kWh_per_min * 60
            )
        else:
    ...
    def max_trip_speed(
        ...
        if end_batt_state is None:
            end_batt_state = self.batt_reserve
        available_charge = self.battery.kWh(start_batt_state) - self.battery.kWh(
            end_batt_state
        )
        if break_number >= 1:
        ...
        if break_number >= 2:
            ...
            available_charge += (break_number - 1) * batt_clone.charge(
